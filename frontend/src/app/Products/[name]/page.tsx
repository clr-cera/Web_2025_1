...
import { useState } from "react"; // Para gerenciar o estado da quantidade
...
export default function ProductPage({ params }: ProductPageProps) {
  const name = params.name; // Acessa diretamente o parâmetro `name`
  ...
  const elementData: ElementProps | undefined = elementsData.find(
    (element) => element.name === name
  );

  // Caso o elemento não seja encontrado, exibe a página 404
  if (!elementData) {
    notFound();
  }
  ...
  return (
    <div className="flex flex-col lg:flex-row mt-10 px-5 lg:px-20 gap-10">
      {/* Imagem do elemento */}
